...
RUN apt update && apt install --assume-yes --no-install-recommends build-essential cmake pkg-config libm4ri-dev libboost-all-dev libjemalloc-dev libtbb-dev && cd xnf_cdcl_solver && cmake . && make xnf_cdcl_solver
