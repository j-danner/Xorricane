...
COPY . /2xnf_solver
RUN apt update && apt install --assume-yes --no-install-recommends build-essential cmake autoconf && cd 2xnf_solver && cmake . && make 2xnf_solver
ENTRYPOINT [ "2xnf_solver/2xnf_solver" ]

# build image with
#   docker build -t 2xnf_solver:0.1 .
