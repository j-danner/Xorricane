...
public:
    ...
    /// @brief Copy all the bits from another bit-vector of the same size into this one.
    /// Will silently fail if this bit-vector is not the exact same size as @p other .
    /// Will silently fail if @p other refers to the same memory as this bit-vector.
    /// @param other The bit-vector to copy.
    void unsafe_copy(const vector& other) noexcept
    {
        static_assert(std::is_trivially_copyable_v<Block>,
                    "unsafe_copy requires trivially-copyable Block");

        std::memcpy(m_blocks.data(),
                    other.m_blocks.data(),
                    m_blocks.size() * sizeof(Block));
    }
    ...
    enum class NoneOrFinal { None, Final, Other };
    constexpr NoneOrFinal none_or_final() const
    {
        auto final_element = size() - 1;
        auto final_block_idx = block_index_for(final_element);
        for (std::size_t i = 0; i < final_block_idx; ++i)
            if (block(i) != 0) return NoneOrFinal::Other;
        auto final_block = block(final_block_idx);
        if (final_block == 0) return NoneOrFinal::None;
        else if (final_block == 1 << bit_index_for(final_element)) return NoneOrFinal::Final;
    }
