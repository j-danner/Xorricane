...
class xlit_watch : public xlit
{
  private:
    ...
    /**
     * @brief dl in which this unit was learnt
     */
    var_t dl = 0;
  ...
  public:
    xlit_watch() {};
    xlit_watch(xlit&& lit, const vec<bool3>& alpha, const vec<var_t>& alpha_dl, const var_t& lvl) : xlit(std::move(lit)), dl(lvl) { init(alpha, alpha_dl); };
    xlit_watch(const xlit& lit, const vec<bool3>& alpha, const vec<var_t>& alpha_dl, const var_t& lvl) : xlit(lit), dl(lvl) { init(alpha, alpha_dl); }
    ...
    var_t get_assigning_lvl(const vec<var_t>& alpha_dl) const {
      return idxs.size()==0 ? dl : std::max( alpha_dl[idxs[ws[0]]], dl );
    };
